...
import { Task, TaskFilter, SortingCriteria, TaskCache } from "./types/TaskIndex";
...
export class TaskManager extends Component {
    ...
    constructor(
        ...
        this.indexer = new TaskIndexer(this.app, this.vault, this.metadataCache);
        ...
        if (this.options.useWorkers) {
            try {
                this.workerManager = new TaskWorkerManager(this.vault, this.metadataCache, {
    ...
    public async initialize(): Promise<void> {
        ...
        try {
            ...
            if (cleared.size > 0) {
                this.log(`Dropped ${cleared.size} out-of-date file task caches`);
            }
        } catch (error) {
            ...
            try {
                ...
                for (let i = 0; i < files.length; i += batchSize) {
                    ...
                    for (const file of batch) {
                        // 尝试从缓存加载
                        try {
                            const cached = await this.persister.loadFile<Task[]>(file.path);
                            if (cached && cached.time >= file.stat.mtime && cached.version === this.version) {
                                // 使用缓存数据更新索引
                                this.indexer.updateIndexWithTasks(file.path, cached.data);
                                this.log(`Loaded ${cached.data.length} tasks from cache for ${file.path}`);
                                cachedCount++;
                            } else {
                ...
                this.log(`Completed worker-based indexing (${importedCount} imported, ${cachedCount} from cache)`);
            } catch (error) {
                console.error("Error using workers for initial indexing:", error);
                this.log("Falling back to single-threaded indexing");
    ...
    private async processFileWithWorker(file: TFile): Promise<void> {
        ...
        try {
            ...
            if (tasks.length > 0) {
                await this.persister.storeFile(file.path, tasks);
                this.log(`Processed and cached ${tasks.length} tasks in ${file.path}`);
            } else {
    ...
    private async fallbackToMainThreadIndexing(files: TFile[]): Promise<void> {
        ...
        for (let i = 0; i < files.length; i += batchSize) {
            ...
            if ((i + batchSize) % 100 === 0 || i + batchSize >= files.length) {
                this.log(`Indexed ${Math.min(i + batchSize, files.length)}/${files.length} files (${Math.round((Math.min(i + batchSize, files.length) / files.length) * 100)}%)`);
            }
        ...
        this.log(`Completed main-thread indexing (${importedCount} imported, ${cachedCount} from cache)`);
    }
    ...
    public async indexFile(file: TFile): Promise<void> {
        ...
        if (this.workerManager) {
            ...
            if (tasks.length > 0) {
                await this.persister.storeFile(file.path, tasks);
                this.log(`Processed ${tasks.length} tasks in ${file.path} using main thread`);
            } else {
     ...
     */
    public queryTasks(filters: TaskFilter[] = [], sortBy: SortingCriteria[] = []): Task[] {
        if (!this.initialized) {
